function log() {
    document.getElementById('results').innerText = '';

    Array.prototype.forEach.call(arguments, function (msg) {
        if (msg instanceof Error) {
            msg = "Error: " + msg.message;
        }
        else if (typeof msg !== 'string') {
            msg = JSON.stringify(msg, null, 2);
        }
        document.getElementById('results').innerHTML += msg + '\r\n';
    });
}

document.getElementById("login").addEventListener("click", login, false);
document.getElementById("api").addEventListener("click", api, false);
document.getElementById("logout").addEventListener("click", logout, false);

var config = {
    authority: "https://localhost:44392",
    client_id: "js",
    redirect_uri: "http://127.0.0.1:5500/callback.html",
    response_type: "code",
    scope: "openid email phone event.graph pushnotification.signalr",
    post_logout_redirect_uri: "http://127.0.0.1:5500/index.html",
};
var mgr = new Oidc.UserManager(config);

mgr.getUser().then(function (user) {
    if (user) {
        log("User logged in", user.profile);
        console.log(user.profile);
        document.getElementById('token').innerText = user.access_token;
    }
    else {
        log("User not logged in");
    }
});

function login() {
    mgr.signinRedirect();
}

function api() {
    mgr.getUser().then(function (user) {
        console.log("Calling API")
        var url = "https://localhost:44313/graphql/";
        var query = {
            query: `{
            eventRegistrations(eventId: "2dfb06d1b56eeb1192588c47be5b2859"){
              nodes{
                id
                eventId
                name
                phoneNumber
              }
            }
          }`};
        console.log(JSON.stringify(query));

        var xhr = new XMLHttpRequest();
        xhr.open("POST", url);
        xhr.onload = function () {
            log(xhr.status, JSON.parse(xhr.responseText));
        }
        xhr.setRequestHeader("Authorization", "Bearer " + user.access_token);
        xhr.send(JSON.stringify(query));
    });
}

function logout() {
    mgr.signoutRedirect();
}