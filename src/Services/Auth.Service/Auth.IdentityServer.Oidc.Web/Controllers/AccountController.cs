...
using Auth.IdentityServer.Oidc.Web.IntegrationEvents;
...
namespace Auth.IdentityServer.Oidc.Web.Controllers
{
    [SecurityHeaders]
    [AllowAnonymous]
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Signup(SignupViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    await _eventBus.PublishAsync(new NewUserSignedUpIntegrationEvent()
                    {
                        UserId = user.Id,
                        Email = user.Email,
                    });
